...
class GridTransform(Node):
    def __init__(self):
        super().__init__("grid_transform", namespace="traversability")

        self.tf_buffer = tf2_ros.Buffer()
        self.tf_listener = tf2_ros.TransformListener(self.tf_buffer, self)

        self.grid_sub = self.create_subscription(
            GridMap16, "heightmap", self.grid_callback, 10
        )

        self.grid_pub = self.create_publisher(GridMap16, "heightmap_odom", 10)
        self.test_pose_pub_tr_ct = self.create_publisher(
            PoseStamped, "transform_pose_center", 10
        )
        self.test_pose_pub_tr = self.create_publisher(PoseStamped, "transform_pose", 10)
    ...
    def transform_grid(self, grid, transform):
        ...
        transformed_grid.data = [-1] * len(grid.data)
